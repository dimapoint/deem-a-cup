            ...
            <div className="rounded-lg border border-gray-800 bg-[#1e232b] p-4">
                ...
                <div className="flex items-end justify-between h-32 gap-1">
                    {[1, 2, 3, 4, 5].map((rating) => {
                        // Aggregate half stars into the main integer star for simplicity in this small view
                        // Or we can show all. Let's show integer buckets for cleaner look if space is tight, 
                        // but showing specific ratings is more accurate.
                        ...
                        return (
                            <div key={rating} className="flex flex-col items-center gap-1 w-full group relative">
                                {/* Tooltip */}
                    ...
                    {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map(day => {
                        ...
                        return (
                            <div key={day} className="flex items-center gap-3 text-xs">
                                <span className={`w-16 ${isMostActive ? 'text-orange-400 font-bold' : 'text-gray-500'}`}>
                                    {day.substring(0, 3)}
                                </span>
